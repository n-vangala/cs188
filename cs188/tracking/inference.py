...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    allPos = [(x, y) for x in range(X_RANGE) for y in range(Y_RANGE)]
    possibleObservations = list(range(X_RANGE + Y_RANGE + MAX_NOISE - 1))

    variableDomainsDict[PAC] = allPos
    variableDomainsDict[GHOST0] = allPos
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        for variable in eliminationOrder:
            ...
            if len(joinedFactor.unconditionedVariables()) > 1:
                factors.append(eliminate(joinedFactor, variable))
        finalJoinFactor = joinFactors(factors)
        return normalize(finalJoinFactor)
        "*** END YOUR CODE HERE ***"
...
class ExactInference(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for ghostPosition in self.allPositions:
            ...
            chanceGivenPrior = self.getObservationProb(observation, pacmanPosition, ghostPosition, jailPosition)
            self.beliefs[ghostPosition] = prior * chanceGivenPrior
        "*** END YOUR CODE HERE ***"
        self.beliefs.normalize()
...
class ParticleFilter(InferenceModule):
    ...
    def observeUpdate(self, observation: int, gameState: busters.GameState):
        ...
        for particle in self.particles:
            observationProbability = self.getObservationProb(observation, pacmanPosition, particle, jailPosition)
            weights[particle] += observationProbability
        if weights.total() == 0:
            self.initializeUniformly(gameState)
        else:
            self.particles = []
    ...
    def elapseTime(self, gameState):
        ...
        for oldPosition in self.particles:
            ...
            newParticles.append(newPosition)
